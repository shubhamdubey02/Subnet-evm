...
AVAGO_DOWNLOAD_URL=https://github.com/cryft-labs/cryftgo/releases/download/${AVALANCHE_VERSION}/avalanchego-linux-${GOARCH}-${AVALANCHE_VERSION}.tar.gz
AVAGO_DOWNLOAD_PATH=${BASEDIR}/avalanchego-linux-${GOARCH}-${AVALANCHE_VERSION}.tar.gz

if [[ ${GOOS} == "darwin" ]]; then
  AVAGO_DOWNLOAD_URL=https://github.com/cryft-labs/cryftgo/releases/download/${AVALANCHE_VERSION}/avalanchego-macos-${AVALANCHE_VERSION}.zip
...
if [[ -f ${AVAGO_DOWNLOAD_PATH} ]]; then
  ...
  if curl -s --head --request GET "${AVAGO_DOWNLOAD_URL}" | grep "302" > /dev/null; then
    ...
    GIT_CLONE_URL=https://github.com/cryft-labs/cryftgo.git
